...
function bp_menu_item_slugs( $menu_items, $args ) {
    ...
    if ( is_user_logged_in() ){
        ...
        if ( strstr( $item->xfn, 'directory-groups' ) ) {
            ...
            if ( is_user_logged_in() ){
	        	if ( strstr( $item->xfn, 'bp-user-directory-my-friends-activity' ) ) {
	        		$menu_items[$key]->url = bp_get_root_domain() . '/' . $bp->activity->root_slug ;
	            }
	
	        	if ( strstr( $item->xfn, 'bp-user-directory-my-groups-activity' ) ) {
	        		$menu_items[$key]->url = bp_get_root_domain() . '/' . $bp->activity->root_slug ;
	            }
	            
	        	if ( strstr( $item->xfn, 'bp-user-directory-my-favorites-activity' ) ) {
	        		$menu_items[$key]->url = bp_get_root_domain() . '/' . $bp->activity->root_slug ;
	            }
	            
	        	if ( strstr( $item->xfn, 'bp-user-directory-my-mentions-activity' ) ) {
	        		$menu_items[$key]->url = bp_get_root_domain() . '/' . $bp->activity->root_slug ;
	            }
	        	if ( strstr( $item->xfn, 'bp-user-directory-my-friends-members' ) ) {
	        		$menu_items[$key]->url = bp_get_root_domain() . '/' . $bp->members->root_slug ;
	            }
		        if ( strstr( $item->xfn, 'bp-user-directory-my-topics-forums' ) ) {
		            $menu_items[$key]->url = bp_get_root_domain() . '/' . $bp->forums->root_slug;
		        }
